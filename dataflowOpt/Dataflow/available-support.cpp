...
namespace llvm {
  ...
  v1{e.v1}, v2{e.v2}, op{e.op}
  {}


  bool Expression::containsOperand(const Value* op) const
  {
    std::string curr_name{op->getName().str()};
    return ((v1->getName().str() == curr_name) || (v2->getName().str() == curr_name));
  }
